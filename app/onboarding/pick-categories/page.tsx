"use client";

import { useEffect, useState } from "react";
import { usePathname, useRouter } from "next/navigation";
import { ArrowRight, Loader2, Tag } from "lucide-react";
import { supabase } from "@/lib/supabase";
import {
  fetchOnboardingContext,
  getDashboardOnboardingGatePath,
  hasBuyerIdentity,
  hasBuyerInterests,
  hasBuyerLocation,
  type ProfileOnboardingRow,
} from "@/lib/onboardingState";
import { getClientUserSafe } from "@/lib/getClientUserSafe";
import {
  BUYER_ONBOARDING_CATEGORY_OPTIONS,
  filterBuyerPickSlugsForStorefront,
} from "@/lib/buyerOnboardingCategories";

type DebugState = {
  userId: string;
  onboardingStep: string;
  onboardingCompleted: boolean;
  isSeller: boolean;
  hasIdentity: boolean;
  hasLocation: boolean;
  hasInterests: boolean;
  currentPath: string;
  computedNext: string;
};

export default function OnboardingPickCategoriesPage() {
  const router = useRouter();
  const pathname = usePathname();
  const [booting, setBooting] = useState(true);
  const [saving, setSaving] = useState(false);
  const [selected, setSelected] = useState<Set<string>>(new Set());
  const [errorMsg, setErrorMsg] = useState("");
  const [debug, setDebug] = useState<DebugState | null>(null);
  const devMode = process.env.NODE_ENV === "development";

  useEffect(() => {
    let cancelled = false;
    (async () => {
      const user = await getClientUserSafe(supabase);
      if (!user) {
        router.replace("/login");
        return;
      }

      const ctx = await fetchOnboardingContext(supabase, user.id);
      if (cancelled) return;

      const p = ctx.profile as ProfileOnboardingRow | null;
      const next = getDashboardOnboardingGatePath(ctx) ?? "/dashboard";
      setDebug({
        userId: user.id,
        onboardingStep: String(p?.onboarding_step || ""),
        onboardingCompleted: p?.onboarding_completed === true,
        isSeller: p?.is_seller === true,
        hasIdentity: hasBuyerIdentity(p),
        hasLocation: hasBuyerLocation(p),
        hasInterests: hasBuyerInterests(p),
        currentPath: pathname || "/onboarding/pick-categories",
        computedNext: next,
      });

      if (!hasBuyerIdentity(p)) {
        router.replace("/onboarding/setup");
        return;
      }
      if (!hasBuyerLocation(p)) {
        router.replace("/onboarding/location");
        return;
      }

      if (!devMode && next !== "/onboarding/pick-categories") {
        router.replace(next);
        return;
      }

      const { data: profile } = await supabase
        .from("profiles")
        .select("buyer_interested_categories")
        .eq("id", user.id)
        .maybeSingle();
      const existing = profile?.buyer_interested_categories;
      if (Array.isArray(existing) && existing.length > 0) {
        setSelected(new Set(existing.map(String)));
      }
      setBooting(false);
    })();
    return () => {
      cancelled = true;
    };
  }, [router, pathname, devMode]);

  const canContinue = selected.size >= 3 && selected.size <= 5;

  const toggle = (slug: string) => {
    setSelected((prev) => {
      const next = new Set(prev);
      if (next.has(slug)) next.delete(slug);
      else if (next.size < 5) next.add(slug);
      return next;
    });
  };

  const handleContinue = async () => {
    const arr = filterBuyerPickSlugsForStorefront(Array.from(selected));
    if (arr.length < 3) {
      setErrorMsg("Pick at least 3 categories (max 5).");
      return;
    }
    setSaving(true);
    setErrorMsg("");
    try {
      const user = await getClientUserSafe(supabase);
      if (!user) {
        router.replace("/login");
        return;
      }
      const { error } = await supabase
        .from("profiles")
        .update({
          buyer_interested_categories: arr,
          onboarding_completed: true,
          onboarding_step: "done",
          updated_at: new Date().toISOString(),
        })
        .eq("id", user.id);
      if (error) throw error;
      router.replace("/dashboard");
      router.refresh();
    } catch (err: unknown) {
      setErrorMsg(err instanceof Error ? err.message : "Could not save categories.");
    } finally {
      setSaving(false);
    }
  };

  if (booting) {
    return (
      <div className="flex min-h-dvh items-center justify-center bg-gray-50">
        <Loader2 className="h-10 w-10 animate-spin text-emerald-600" />
      </div>
    );
  }

  return (
    <div className="min-h-dvh bg-gray-50 flex flex-col items-center p-6 pb-24">
      {devMode ? (
        <div className="w-full max-w-3xl mb-4 space-y-2 rounded-xl border border-emerald-200 bg-emerald-50 p-3">
          <p className="text-[10px] font-black uppercase tracking-widest text-emerald-700">Onboarding debug</p>
          <div className="grid grid-cols-1 gap-1 text-[11px] font-semibold text-emerald-900 md:grid-cols-2">
            <p>user_id: <span className="font-mono">{debug?.userId || "-"}</span></p>
            <p>current_path: <span className="font-mono">{debug?.currentPath || "-"}</span></p>
            <p>onboarding_step: <span className="font-mono">{debug?.onboardingStep || "(empty)"}</span></p>
            <p>onboarding_completed: <span className="font-mono">{String(debug?.onboardingCompleted)}</span></p>
            <p>is_seller: <span className="font-mono">{String(debug?.isSeller)}</span></p>
            <p>computed_next: <span className="font-mono">{debug?.computedNext || "-"}</span></p>
            <p>has_identity: <span className="font-mono">{String(debug?.hasIdentity)}</span></p>
            <p>has_location: <span className="font-mono">{String(debug?.hasLocation)}</span></p>
            <p>has_interests: <span className="font-mono">{String(debug?.hasInterests)}</span></p>
          </div>
        </div>
      ) : null}

      <div className="w-full max-w-xl pt-2">
        <p className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-400 text-center mb-3">
          StoreLink · Shared Step · Pick Categories
        </p>
        <div className="flex justify-center mb-4">
          <span className="inline-flex items-center gap-2 bg-emerald-50 text-emerald-700 border border-emerald-100 px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest">
            <Tag size={12} /> Pick interests
          </span>
        </div>
        <h1 className="text-3xl font-black text-gray-900 text-center uppercase tracking-tighter mb-2">What do you like?</h1>
        <p className="text-sm text-gray-500 text-center mb-8 font-medium leading-relaxed">
          Choose 3-5 categories to complete onboarding.
        </p>

        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-8">
          {BUYER_ONBOARDING_CATEGORY_OPTIONS.map((cat) => {
            const on = selected.has(cat.slug);
            return (
              <button
                key={cat.slug}
                type="button"
                onClick={() => toggle(cat.slug)}
                className={`rounded-2xl border px-4 py-4 text-left font-bold text-sm transition shadow-sm ${
                  on
                    ? "border-emerald-500 bg-emerald-50 text-emerald-900 ring-2 ring-emerald-200"
                    : "border-gray-100 bg-white text-gray-800 hover:border-gray-200"
                }`}
              >
                {cat.label}
              </button>
            );
          })}
        </div>

        {errorMsg ? <p className="text-xs font-bold text-red-600 text-center mb-4">{errorMsg}</p> : null}
        <p className="text-[11px] text-gray-400 text-center mb-6 font-medium">{selected.size} selected · need 3-5</p>

        <button
          type="button"
          disabled={saving || !canContinue}
          onClick={handleContinue}
          className="w-full py-4 rounded-2xl bg-gray-900 text-white font-black text-[10px] uppercase tracking-[0.2em] shadow-lg flex items-center justify-center gap-2 disabled:opacity-50"
        >
          {saving ? (
            <Loader2 className="animate-spin" size={18} />
          ) : (
            <>
              Continue <ArrowRight size={18} />
            </>
          )}
        </button>
      </div>
    </div>
  );
}
