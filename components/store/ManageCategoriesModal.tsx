...
export default function ManageCategoriesModal({ sellerId, isOpen, onClose }: ManageCategoriesModalProps) {
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    const baseSlug = newCategoryName
      .trim()
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/^-|-$/g, "");
    const slug = `${sellerId.slice(0, 8)}-${baseSlug}-${Date.now()}`.slice(0, 120);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
      <div className="bg-white w-full max-w-sm rounded-3xl overflow-hidden shadow-2xl animate-in zoom-in-95">
        <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-gray-50">
          ...
          <button onClick={onClose} className="p-2 bg-white rounded-full shadow-sm text-gray-500 hover:bg-gray-100">
            <X size={20} />
          </button>
        </div>
